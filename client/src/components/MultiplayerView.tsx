...
const MODES: { icon: typeof Globe; mode: GameMode; title: string; desc: string }[] = [
  {
    icon: Globe,
    mode: "classic",
...
export function MultiplayerView({ onNavigate, onSelectMode }: MultiplayerViewProps) {
  return (
      ...
      <div className="mx-auto flex min-h-dvh w-full max-w-md flex-col justify-center p-4">
        <Card className="border-white/30 shadow-2xl">
          ...
          <CardContent className="space-y-3">
            {MODES.map(({ icon: Icon, mode, title, desc }) => (
              <button
                key={title}
